...
public class MoveHandler {
    ...
    public void handle(MakeMoveCommand command, Session session) throws DataAccessException {
        try {
            ...
            LoadGameMessage updatedGameMessage = new LoadGameMessage(gameLogic);
            connections.broadcast(gameID, updatedGameMessage, null); // Send to all (no exclusions)

            // Notify clients
            String moveDescription = user.getUsername() + " moved from " + move.getStartPosition() + " to " + move.getEndPosition();
